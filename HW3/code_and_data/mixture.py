...
class Model(object):
    ...
    def update_pi(self, R):
        ...
        # R.shape -> n, k (no. samples, no. classes)
        n = R.shape[0]
        k = R.shape[1]
        
        # calculating the numerator {sumi_r + a - 1}
        # since we have to sum over all samples, and the rows contain the samples, we sum over all the rows
        sumi_r = R.sum(axis=0)
        num = sumi_r + self.prior.a_mix - 1

        # calculting the denominator {sumk_sumi_r + ka - k}
        # sum over all rows, then sum over all the columns 
        sumk_sumi_r = R.sum(axis=0).sum()
        den = sumk_sumi_r + (k*self.prior.a_mix) - k
    ...
    def update_theta(self, X, R):
        ...
        # R.shape -> n, k (no. samples, no. classes)
        # X.shape -> n, d (no. samples, no. features/pixels)
        RT = np.transpose(R)

        # calculating {sumi_r*x + (a-1)}
        sumi_rx = np.matmul(RT, X) # shape -> (kxn).(nxd) = (kxd) 
        a_term = sumi_rx + (self.prior.a_pixels - 1)

        # calculating {sumi_r*(1-x) + (b-1)}
        sumi_r1minusx = np.matmul(RT, (1-X)) # shape -> (kxn).(nxd) = (kxd)
        b_term = sumi_r1minusx + (self.prior.b_pixels - 1) 

        thetakj = a_term/(a_term + b_term)
        return thetakj
    ...
    def compute_posterior(self, X, M=None):
        ...
        # say, a = [1, 2, 3] and obs = [0, 1, 0], then, a*obs = [0, 2, 0], ie, only the observed elements in a
        x_obs = X * M 

        # log p(zi = k, xi) = log p(zi = k) + log p(xi | zi = k)
        # now, log p(zi = k) = log(pik)
        # and, log p(xi | zi = k) = log(productj(thetakj^xi * (1-thetakj)^(1-xi))
        #                         = sumj(xi*log(thetakj) + (1-xi)*log(1-thetakj))

        # calculating log(pik)
        log_pik = np.log(self.params.pi)

        # calculating xi*log(thetakj)
        log_thetakj = np.log(self.params.theta)
        term1 = np.matmul(x_obs, np.transpose(log_thetakj))

        # calculating (1-xi)*log(1-thetakj)
        log_1minusthetakj = np.log(1 - self.params.theta)
        term2 = np.matmul((1-x_obs), np.transpose(log_1minusthetakj))
        
        # calculating log(p(zi = k, xi))
        log_p_z_x = log_pik + term1 + term2
    ...
    def posterior_predictive_means(self, X, M):
        ...
        post_pred = np.matmul(post, self.params.theta)
        return post_pred
...
#### UNCOMMENT THE FOLLOWING FUNCTIONS TO DISPLAY OUTPUTS FOR THE CORRESPONDING PARTS ########
# print_part_1_values()
# print_part_2_values()
